CLASS zcl_991_democlass_examples DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ZCL_991_DEMOCLASS_EXAMPLES IMPLEMENTATION.
  ...
  METHOD if_oo_adt_classrun~main.
    ...
    DATA(oref) = NEW zcl_991_democlass_examples( ).
    ...
    DATA(oref2) = NEW zcl_991_democlass_examples( ).
    out->write( data = oref2->inst_attr name = `oref2->inst_attr` ).

    DATA(oref3) = NEW zcl_991_democlass_examples( ).
    ...
    "the attribute name.
    out->write( data = zcl_991_democlass_examples=>stat_attr name = `zcl_991_democlass_examples=>stat_attr` ).
    ...
    AND ( oref->stat_attr = zcl_991_democlass_examples=>stat_attr ).
    ...
    DATA(d) = zcl_991_democlass_examples=>stat_attr.
    ...
    DATA(res) = zcl_991_democlass_examples=>stat_meth1( ip1 = `a` "pass by reference (declaration with REFERENCE(...))
    ...
    DATA(res_tab1) = zcl_991_democlass_examples=>stat_meth2( ip1 = `aaa` ip2 = `bbb` ).
    out->write( data = res_tab1 name = `res_tab1` ).

    DATA(res_tab2) = zcl_991_democlass_examples=>stat_meth2( ip1 = `ccc` ).
    out->write( data = res_tab2 name = `res_tab2` ).

    DATA(res_tab3) = zcl_991_democlass_examples=>stat_meth2( ip2 = `ddd` ).
    out->write( data = res_tab3 name = `res_tab3` ).

    DATA(res_tab4) = zcl_991_democlass_examples=>stat_meth2( ).
    ...
    DATA(res_gen1) = zcl_991_democlass_examples=>stat_meth3( ip_data = VALUE string_table( ( `hi` ) ) "any data type
    ...
    DATA(res_gen2) = zcl_991_democlass_examples=>stat_meth3( ip_data = 123
                                                ip_clike = 'ABAP'
